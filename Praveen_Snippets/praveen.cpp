...
ll modinv(ll p, ll q, ll mod) {ll ex; ex = mod - 2; while (ex) {if (ex & 1) {p = (p * q) % mod;} q = (q * q) % mod; ex >>= 1;} return p;}
ll binaryExponentiation(ll base, ll index, ll MOD) {ll ans = 1; while (index > 0) {if (index & 1) {ans = (ans * base) % MOD;} base = (base * base) % MOD; index >>= 1;} return ans;}
...
void solve() {
	...
	for (int j = 1; j <= n; ++j) {
		sum += a[j - 1];
		if (mp.find(sum - tot) != mp.end()) {
			int len = j - mp[sum - tot];
			if (len < ans) ans = len;
		}
		mp[sum] = j;
	}
